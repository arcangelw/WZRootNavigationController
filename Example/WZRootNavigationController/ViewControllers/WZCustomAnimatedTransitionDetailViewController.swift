//
//  WZCustomAnimatedTransitionDetailViewController.swift
...
class WZCustomAnimatedTransitionDetailViewController: UIViewController {
    @IBOutlet weak var imageView: UIImageView!
    var fromSelected:IndexPath?
...
class WZCustomAnimatedTransition: WZAnimatedTransitionPlugin {
    ...
    override func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if self.operation == .push ,from.contentViewController is WZCustomAnimatedTransitionListCollectionViewController {
            ...
            guard let selected = fromContent.collectionView!.indexPathsForSelectedItems?.first,
                let cell = fromContent.collectionView!.cellForItem(at: selected) as? WZCustomAnimatedTransitionListCell
                else {
                    super.animateTransition(using: transitionContext)
                    return
                    }
            ...
            guard let selected = fromContent.fromSelected,
                let cell = toContent.collectionView!.cellForItem(at: selected) as? WZCustomAnimatedTransitionListCell
                else {
                    super.animateTransition(using: transitionContext)
                    return
            }
